...
export default function KumasArsiviSayfasi() {
    ...
    const yukle = async () => {
        ...
        try {
            if (sekme === 'kumas') {
                ...
                if (kumasRes.status === 'fulfilled' && kumasRes.value.data) setKumaslar(kumasRes.value.data);
                ...
                // Hata yönetimini Promise.allSettled'a göre yap (Çökmeyi engelle)
                const kData = (kumasRes.status === 'fulfilled' && kumasRes.value?.data) ? kumasRes.value.data : [];
                const aData = (aksesuarRes.status === 'fulfilled' && aksesuarRes.value?.data) ? aksesuarRes.value.data : [];
    ...
    const sil = async (tablo, id) => {
        ...
        if (!kullanici || kullanici.grup !== 'tam') {
            const adminPin = prompt('Bu veriyi Silmek için Yönetici PİN kodunu girin:');
            // Playwright veya tarayıcı engelleyicilerine karşı prompt null dönerse iptal et.
        ...
        try {
            ...
            if (tablo === 'b1_kumas_arsivi') telegramBildirim(`🗑️ KUMAŞ SİLİNDİ\nBir kumaş arşivi veritabanından kalıcı olarak silindi!`);
        } catch (error) {
    ...
    if (!yetkiliMi) {
        return (
            <div style={{ padding: '3rem', textAlign: 'center', background: '#fef2f2', border: '2px solid #fecaca', borderRadius: '16px', margin: '2rem' }}>
                ...
                <p style={{ color: '#7f1d1d', fontWeight: 600, marginTop: 8 }}>Kumaş Arşivi verileri gizlidir. Görüntülemek için Üretim PİN veya Yetkili Kullanıcı girişi gereklidir.</p>
            </div>
        );
    }
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                    ...
                    <div>
                        <h1 style={{ fontSize: '1.4rem', fontWeight: 900, color: '#0f172a', margin: 0 }}>
                            {isAR ? 'أرشيف الأقمشة والمواد' : 'Kumaş & Materyal Arşivi'}
                        </h1>
                        <p style={{ fontSize: '0.78rem', color: '#64748b', margin: '2px 0 0', fontWeight: 600 }}>
                            {isAR ? 'كتالوج رقمي — يتم اختيار القماش المناسب لكل نموذج من هنا' : 'Dijital kartela — modele hangi kumaşın gideceği buradan seçilir'}
                        </p>
                    </div>
                </div>
                ...
                <div style={{ background: 'white', border: '2px solid #047857', borderRadius: 16, padding: '1.5rem', marginBottom: '1.5rem', boxShadow: '0 8px 32px rgba(4,120,87,0.12)' }}>
                    ...
                    <div style={{ display: 'flex', gap: '0.75rem', marginTop: '1rem', justifyContent: 'flex-end' }}>
                        ...
                        <button onClick={kaydetAksesuar} disabled={loading}
                            style={{ padding: '9px 24px', background: loading ? '#94a3b8' : '#8b5cf6', color: 'white', border: 'none', borderRadius: 8, fontWeight: 800, cursor: loading ? 'not-allowed' : 'pointer' }}>
                            {loading ? '...' : (duzenleId ? 'Güncelle' : (isAR ? 'حفظ' : 'Kaydet'))}
                        </button>
                    </div>
                </div>
            ...
            {sekme === 'kumas' && (
                <div>
                                ...
                                <div key={k.id} style={{ background: 'white', border: '2px solid', borderColor: dusukStok ? '#ef4444' : '#f1f5f9', borderRadius: 14, overflow: 'hidden', boxShadow: '0 2px 8px rgba(0,0,0,0.04)', transition: 'all 0.2s' }}>
                                    ...
                                    {k.fotograf_url ? (
                                        <div style={{ height: 140, overflow: 'hidden', background: '#f8fafc' }}>
                                            <img src={k.fotograf_url} alt={k.kumas_adi} style={{ width: '100%', height: '100%', objectFit: 'cover' }} onError={e => e.target.style.display = 'none'} />
                                        </div>
                                    ) : (
                                    ...
                                    <div style={{ padding: '1rem' }}>
                                        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '0.5rem' }}>
                                            <div>
                                                <span style={{ fontSize: '0.65rem', fontWeight: 800, background: '#eff6ff', color: '#2563eb', padding: '2px 8px', borderRadius: 4 }}>{k.kumas_kodu}</span>
                                            ...
                                            <div style={{ display: 'flex', gap: 4 }}>
                                                ...
                                                <button onClick={() => duzenleKumas(k)} style={{ background: '#eff6ff', border: 'none', color: '#2563eb', padding: '4px 8px', borderRadius: 6, cursor: 'pointer', fontSize: '0.68rem', fontWeight: 700 }}>✏️</button>
                    ...
                    <div style={{ display: 'flex', gap: '1rem', marginTop: '1.25rem', flexWrap: 'wrap' }}>
                        {[
                            { label: 'Toplam Kumaş', val: kumaslar.length, color: '#3b82f6' },
                            ...
                            return (
                                <div key={a.id} style={{ background: 'white', border: '2px solid', borderColor: dusuk ? '#ef4444' : '#f1f5f9', borderRadius: 14, padding: '1.25rem', boxShadow: '0 2px 8px rgba(0,0,0,0.04)' }}>
                                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
                                        <div>
                                            <span style={{ fontSize: '0.65rem', fontWeight: 800, background: '#f3e8ff', color: '#7c3aed', padding: '2px 8px', borderRadius: 4 }}>{a.aksesuar_kodu}</span>
                                    ...
                                    <div style={{ display: 'flex', gap: '0.375rem', margin: '0.5rem 0', flexWrap: 'wrap' }}>
                                        <span style={{ fontSize: '0.7rem', background: '#f3e8ff', color: '#7c3aed', padding: '2px 8px', borderRadius: 4, fontWeight: 600 }}>{a.tip}</span>
                        ...
                        {gorselArşiv.map(g => (
                            <div key={g.id} style={{ background: 'white', border: '1px solid #f1f5f9', borderRadius: 12, overflow: 'hidden', boxShadow: '0 2px 8px rgba(0,0,0,0.06)' }}>
                                <div style={{ height: 160, background: '#f8fafc', overflow: 'hidden' }}>
                                    <img src={g.fotograf_url} alt={g.ad} style={{ width: '100%', height: '100%', objectFit: 'cover' }} onError={e => { e.target.parentElement.style.display = 'flex'; e.target.parentElement.style.alignItems = 'center'; e.target.parentElement.style.justifyContent = 'center'; e.target.style.display = 'none'; }} />
                                </div>
                                <div style={{ padding: '0.75rem' }}>
                                    <span style={{ fontSize: '0.6rem', fontWeight: 800, background: g.tip === 'kumas' ? '#eff6ff' : '#f3e8ff', color: g.tip === 'kumas' ? '#2563eb' : '#7c3aed', padding: '2px 6px', borderRadius: 4 }}>{g.tip === 'kumas' ? 'KUMAŞ' : 'AKSESUAR'}</span>
