...
export default function MaliyetSayfasi() {
    ...
    const sil = async (id) => {
        if (!kullanici || kullanici.grup !== 'tam') {
            const pin = prompt('Maliyet kalemini silmek için Yönetici PİN girin:');
            if (pin !== (process.env.NEXT_PUBLIC_ADMIN_PIN || '9999')) return goster('Hatalı yetki!', 'error');
        }
    ...
    const tumunuSil = async () => {
        if (!kullanici || kullanici.grup !== 'tam') {
            const pin = prompt('TÜM kayıtları silmek için Yönetici PİN girin:');
            if (pin !== (process.env.NEXT_PUBLIC_ADMIN_PIN || '9999')) return goster('Hatalı yetki!', 'error');
        }
