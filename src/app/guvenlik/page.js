...
export default function GuvenlikSayfasi() {
    ...
    const handlePinDegistir = () => {
        ...
        try {
            ...
            if (mevcut && mevcut !== pinDegistir.eskiPin && pinDegistir.eskiPin !== process.env.NEXT_PUBLIC_ADMIN_PIN) {
                ...
                return goster('Mevcut kod hatalı!', 'error');
            }
    ...
    const logTemizle = () => {
        // 🛑 AA Kriteri: İzinsiz Silme Engeli (Patron şifresi sor)
        const adminPin = prompt('Güvenlik loglarını silmek tehlikelidir. Yönetici PİN kodunu girin:');
        const dogruPin = process.env.NEXT_PUBLIC_ADMIN_PIN || '9999';
        if (adminPin !== dogruPin) return goster('Yetkisiz İşlem! Loglar silinemedi.', 'error');
