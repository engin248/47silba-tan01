...
const TIP_─░KON = {
    dusuk_stok: { ikon: <Package size={14} />, etiket: 'D├╝┼ş├╝k Stok' },
    maliyet_asimi: { ikon: <TrendingUp size={14} />, etiket: 'Maliyet A┼ş─▒m─▒' },
    fire_yuksek: { ikon: <AlertTriangle size={14} />, etiket: 'Fire Y├╝ksek' },
...
export default function DenetmenSayfasi() {
    ...
    const yukle = async () => {
        ...
        } catch (error) { setMesaj('Veriler y├╝klenemedi: ' + error.message); }
        setLoading(false);
    };

    const taramaCalistir = async () => {
        if (tarama) return goster('ÔÅ│ Tarama zaten devam ediyor...', 'error');
        setTarama(true);
        setMesaj('');
        try {
            // Stok tarama
                ...
                if (u.stok_adeti <= u.min_stok_alarm) {
                    ...
                    if (!var_?.length) {
                        await supabase.from('b1_sistem_uyarilari').insert([{
                            ...
                            baslik: `${u.stok_adeti === 0 ? 'Stok S─▒f─▒r' : 'D├╝┼ş├╝k Stok'}: ${u.urun_adi_tr}`,
                ...
                if (yuzde > 10) {
                    ...
                    if (!var_?.length) {
                        await supabase.from('b1_sistem_uyarilari').insert([{
                            ...
                            baslik: `Maliyet A┼ş─▒m─▒: %${yuzde.toFixed(1)}`,
                            mesaj: `Hedef: Ôé║${hedef.toFixed(0)} | Ger├ğek: Ôé║${parseFloat(r.gerceklesen_maliyet_tl).toFixed(0)} | Fark: +Ôé║${fark.toFixed(0)}`,
            ...
            setMesaj(`Tarama tamamland─▒. ${yeniUyari} yeni uya r─▒ olu┼şturuldu.`);
            yukle();
        } catch (e) {
            setMesaj('Tarama hatas─▒: ' + e.message);
        } finally {
            setTimeout(() => setTarama(false), 3000); // 3 saniye anti-spam bekleme s├╝resi
        }
    };

    const aiAnalizYap = async () => {
        if (uyarilar.length === 0) { setMesaj('Uyar─▒ yok, ├Ânce Tara & G├╝ncelle\'ye bas─▒n.'); return; }
        if (aiYukleniyor) return goster('ÔÅ│ AI zaten analiz ediyor...', 'error');
        ...
        try {
            ...
            const res = await fetch('/api/trend-ara', {
                ...
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({
                    sorgu: `Bir tekstil at├Âlyesinde ┼şu sistem uyar─▒lar─▒ var:\n${ozet}\n\nBunlar─▒ ├Âncelik s─▒ras─▒na g├Âre de─şerlendir ve k─▒sa T├╝rk├ğe ├ğ├Âz├╝m ├Ânerileri sun. Sadece pratik ve uygulanabilir ├Âneriler ver.`
                }),
            });
            const data = await res.json();
            setAiAnaliz(data.ozet || data.sonuclar?.[0]?.aciklama || 'Analiz tamamland─▒.');
        } catch (e) {
            setAiAnaliz('Ba─şlant─▒ hatas─▒: ' + e.message);
        } finally {
    ...
    const coz = async (id, baslik) => {
        try {
            ...
            telegramBildirim(`Ô£à ALARM ├ç├ûZ├£LD├£\nM├╝fetti┼ş: ${baslik}`);
    ...
    if (!yetkiliMi) {
        return (
            <div dir={isAR ? 'rtl' : 'ltr'} style={{ padding: '3rem', textAlign: 'center', background: '#fef2f2', border: '2px solid #fecaca', borderRadius: '16px', margin: '2rem' }}>
                ...
                <p style={{ color: '#7f1d1d', fontWeight: 600, marginTop: 8 }}>Sistem denetmeni verileri son derece gizlidir. G├Âr├╝nt├╝lemek i├ğin Yetkili Kullan─▒c─▒ giri┼şi gereklidir.</p>
            </div>
        );
    }
        ...
        <div>
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1.5rem' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                    ...
                    <div>
                        ...
                        <p style={{ fontSize: '0.78rem', color: '#64748b', margin: '2px 0 0', fontWeight: 600 }}>Otomatik alarm merkezi ÔÇö ger├ğek veri</p>
                    </div>
                </div>
                <div style={{ display: 'flex', gap: 8 }}>
                    <button onClick={taramaCalistir} disabled={tarama}
                        ...
                        {tarama ? 'Taran─▒yor...' : 'Tara & G├╝ncelle'}
                    </button>
                    {/* VISION M├£FETT─░┼Ş (G├ûREV 4) */}
                    <button onClick={() => { setVisionModalAcik(true); setVisionFotoSecili(null); setVisionSonuc(null); }}
                        ...
                        <ScanEye size={18} /> Kuma┼ş / Dikim Analiz
                    </button>
                    <button onClick={aiAnalizYap} disabled={aiYukleniyor || uyarilar.length === 0}
                        ...
                        ­şñû {aiYukleniyor ? 'D├╝┼ş├╝n├╝yor...' : 'Gemini AI Analizi'}
                    </button>
                </div>
            </div>
            ...
            {aiAnaliz && (
                <div style={{ background: 'linear-gradient(135deg,#0f172a,#1e1b4b)', borderRadius: 14, padding: '1rem 1.25rem', marginBottom: '1.25rem', display: 'flex', gap: 12, alignItems: 'flex-start' }}>
                    <span style={{ fontSize: '1.5rem', flexShrink: 0 }}>­şñû</span>
                    <div>
                        <div style={{ fontSize: '0.65rem', fontWeight: 800, color: '#a78bfa', textTransform: 'uppercase', marginBottom: 6 }}>Gemini Otonom Kararg├óh Zekas─▒</div>
            ...
            <div style={{ display: 'flex', gap: '0.375rem', marginBottom: '1rem', flexWrap: 'wrap' }}>
                {['hepsi', 'kritik', 'uyari', 'bilgi', 'dusuk_stok', 'maliyet_asimi', 'video_eksik', 'diger'].map(f => (
                    <button key={f} onClick={() => setFiltre(f)}
                        ...
                        {f === 'hepsi' ? 'T├╝m├╝' : f === 'dusuk_stok' ? 'Stok' : f === 'maliyet_asimi' ? 'Maliyet' : f === 'video_eksik' ? 'Video' : f.charAt(0).toUpperCase() + f.slice(1)}
                    </button>
                ))}
            </div>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem', marginBottom: '1.5rem' }}>
                {loading && <div style={{ textAlign: 'center', padding: '3rem', color: '#94a3b8' }}>Y├╝kleniyor...</div>}
                ...
                {filtrelendi.map(u => {
                    const r = SEV─░YE_RENK[u.seviye] || SEV─░YE_RENK.bilgi;
                    const tip = TIP_─░KON[u.uyari_tipi] || TIP_─░KON.diger;
                    return (
                        <div key={u.id} style={{ background: r.bg, border: `2px solid ${r.border}`, borderRadius: 12, padding: '0.875rem 1.25rem', display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: 12 }}>
                            ...
                            <div style={{ display: 'flex', gap: 6, flexShrink: 0 }}>
                                <button onClick={() => coz(u.id, u.baslik)} title="├ç├Âz├╝ld├╝"
                                    ...
                                    <CheckCircle size={12} /> ├ç├Âz├╝ld├╝
                                </button>
                                <button onClick={() => gozArd(u.id)} title="G├Âz Ard─▒"
            ...
            {/* AI VISION (G├ûR├£) ANAL─░Z MODALI */}
            <SilBastanModal title="­şôÀ Yapay Zeka Vision Analiz ├çekirde─şi" acik={visionModalAcik} onClose={() => !visionYukleniyor && setVisionModalAcik(false)}>
                <div style={{ padding: '0.5rem', textAlign: 'center' }}>
                    ...
                    {!visionFotoSecili ? (
                        <div style={{ border: '3px dashed #cbd5e1', borderRadius: '16px', padding: '3rem 1rem', cursor: 'pointer', background: '#f8fafc', transition: 'all 0.2s', position: 'relative' }}>
                            ...
                            <h3 style={{ margin: '0 0 0.5rem', color: '#334155', fontWeight: 800 }}>Kameray─▒ A├ğ veya Foto─şraf Y├╝kle</h3>
                            <p style={{ margin: 0, color: '#94a3b8', fontSize: '0.85rem', fontWeight: 600 }}>Dikim hatas─▒n─▒ veya kuma┼ş abraj─▒n─▒ buraya tarat─▒n.</p>
                        </div>
                    ) : (
                        <div>
                            {/* Y├╝kl├╝ Resim G├Âsterimi */}
                            <img src={visionFotoSecili} alt="Y├╝klenen Kuma┼ş" style={{ width: '100%', maxHeight: '300px', objectFit: 'contain', borderRadius: '12px', border: '2px solid #e2e8f0', marginBottom: '1.5rem' }} />

                            {/* Sonu├ğ Yoksa Analiz Et Butonu */}
                            {!visionSonuc && (
                                <button
                                    onClick={async () => {
                                ...
                                >
                                    {visionYukleniyor ? '­şñû ├çekirdek Analiz Ediyor (L├╝tfen Bekleyin)...' : 'HATA ORANI ANAL─░Z─░N─░ BA┼ŞLAT'}
                                </button>
                            )}
                                ...
                                <div style={{ background: visionSonuc.onay ? '#f0fdf4' : '#fef2f2', border: `3px solid ${visionSonuc.onay ? '#22c55e' : '#ef4444'}`, borderRadius: '16px', padding: '1.5rem', textAlign: 'left', animation: 'slideIn 0.3s ease-out' }}>
                                    ...
                                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1rem', borderBottom: `2px solid ${visionSonuc.onay ? '#bbf7d0' : '#fca5a5'}`, paddingBottom: '1rem' }}>
                                        <div style={{ fontSize: '1.25rem', fontWeight: 900, color: visionSonuc.onay ? '#166534' : '#991b1b', textTransform: 'uppercase' }}>
                                            {visionSonuc.onay ? 'Ô£à ONAYLANDI (TEM─░Z)' : 'ÔØî REDDED─░LD─░ (DEFOLU)'}
                                        </div>
                                    ...
                                    <div style={{ fontSize: '0.9rem', color: '#334155', fontWeight: 700, lineHeight: 1.6 }}>
                                        <strong style={{ color: '#0f172a' }}>M├╝fetti┼ş Yorumu:</strong><br />
                                        {visionSonuc.yorum}
                                    </div>
