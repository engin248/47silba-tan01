...
export default function ModelhaneSayfasi() {
        ...
        try {
            ...
            if (sekme === 'numuneler') {
                const [nRes, mRes] = await Promise.race([
                    Promise.allSettled([
                        supabase.from('b1_numune_uretimleri').select('*, b1_model_taslaklari(model_adi,model_kodu), b1_model_kaliplari(kalip_adi,versiyon)').order('created_at', { ascending: false }).limit(200),
                ...
                const [tRes, nRes] = await Promise.race([
                    Promise.allSettled([
                        supabase.from('b1_dikim_talimatlari').select('*, b1_numune_uretimleri(numune_beden, b1_model_taslaklari(model_adi,model_kodu))').order('created_at', { ascending: false }).limit(200),
                ...
                const [gRes, tumNRes, mRes] = await Promise.race([
                    Promise.allSettled([
                        supabase.from('b1_numune_uretimleri').select('id, numune_beden, onay_durumu, fotograflar, b1_model_taslaklari(model_kodu, model_adi)').order('created_at', { ascending: false }).limit(200),
    ...
    const adimEkle = () => {
        ...
        setFormT(prev => ({ ...prev, yazili_adimlari: [...prev.yazili_adimlari, { adim_no: yeniNo, aciklama: '', sure_dk: 5 }] }));
    };
    ...
    const sil = async (tablo, id) => {
        // AA KRİTERİ ONARIMI: Yetkisiz Silme Engeli
        if (!kullanici || kullanici.grup !== 'tam') {
            const adminPin = prompt('Sadece TAM YETKİLİ personeller VEYA Şifre sahipleri silebilir.\nLütfen PİN kodunuzu girin:');
            const dogruPin = process.env.NEXT_PUBLIC_ADMIN_PIN || '9999';
            if (adminPin !== dogruPin) return goster('Hatalı Yetki Kodu! İşlem Engellendi.', 'error');
        }

        if (!confirm(isAR ? 'هل أنت متأكد من الحذف؟' : 'Veri kalıcı olarak silinsin mi?')) return;
        try {
            ...
            if (!error) { yukle(); goster('Kayıt güvenle silindi.'); }
            else throw error;
                        ...
                        <div>
                            ...
                            <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem' }}>
                                {formT.yazili_adimlari.map((adim, i) => (
                                    <div key={i} style={{ display: 'grid', gridTemplateColumns: '40px 1fr 80px 32px', gap: '0.375rem', alignItems: 'center', background: '#f8fafc', padding: '8px', borderRadius: 8 }}>
                                        ...
                                        <input type="number" value={adim.sure_dk} onChange={e => adimGuncelle(i, 'sure_dk', e.target.value)} placeholder="dk" style={{ ...inp, padding: '6px 10px', textAlign: 'center' }} min={1} />
                                        <button onClick={() => adimSil(i)} style={{ background: '#fef2f2', border: 'none', color: '#dc2626', padding: 6, borderRadius: 6, cursor: 'pointer' }}><Trash2 size={12} /></button>
                                    </div>
                                ))}
                            </div>
                        </div>
                                    ...
                                    <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill,minmax(180px,1fr))', gap: '0.75rem' }}>
                                        {(n.fotograflar || []).map((url, i) => (
                                            <div key={i} style={{ position: 'relative', borderRadius: 10, overflow: 'hidden', border: '2px solid #e5e7eb' }}>
                                                <img src={url} alt={`Foto ${i + 1}`} style={{ width: '100%', height: 200, objectFit: 'cover', cursor: 'zoom-in', display: 'block' }} onClick={() => window.open(url, '_blank')} />
