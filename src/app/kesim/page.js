...
const BOSH_KESIM = {
    ...
    kesimci_adi: '', kesim_tarihi: '', beden_dagilimi: '', notlar: '', kumas_topu_no: ''
};
...
export default function KesimhaneSayfasi() {
    ...
    const kaydetKesim = async () => {
        ...
        const payload = {
            ...
            beden_dagilimi: form.beden_dagilimi.trim() || null,
    ...
    const sil = async (id, m_kodu) => {
        if (!kullanici || kullanici.grup !== 'tam') {
            const adminPin = prompt('Kesim kaydını silmek için Yönetici PİN girin:');
        ...
        try {
            try {
                await supabase.from('b0_sistem_loglari').insert([{
                    tablo_adi: 'b1_kesim_operasyonlari', islem_tipi: 'SILME', kullanici_adi: 'Saha Yetkilisi M3',
                    ...
                    <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(220px, 1fr))', gap: '1rem' }}>
                        ...
                        <div style={{ gridColumn: '1 / -1' }}>
                            ...
                            <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap', marginBottom: 8 }}>
                                {BEDENLER.map(b => {
                                    const secili = form.beden_dagilimi.includes(b);
                                    return (
                                        <button key={b} type="button" onClick={() => {
                                            const mevcutlar = form.beden_dagilimi ? form.beden_dagilimi.split(',').map(x => x.trim()).filter(Boolean) : [];
                                            const yeni = secili ? mevcutlar.filter(x => x !== b) : [...mevcutlar, b];
                                            setForm({ ...form, beden_dagilimi: yeni.join(', ') });
                                        }}
                                            style={{ padding: '5px 12px', border: '2px solid', borderColor: secili ? '#047857' : '#e5e7eb', background: secili ? '#047857' : 'white', color: secili ? 'white' : '#374151', borderRadius: 8, fontWeight: 800, cursor: 'pointer', fontSize: '0.8rem' }}>
                                            {b}
                                        </button>
                                    );
                                })}
                            </div>
                            <input type="text" value={form.beden_dagilimi} onChange={e => setForm({ ...form, beden_dagilimi: e.target.value })}
                                placeholder="Örn: S-120, M-200, L-180, XL-100" style={{ ...inp, fontSize: '0.82rem' }} />
                        </div>
                            ...
                            <div style={{ padding: '1.25rem' }}>
                                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '0.75rem' }}>
                                    ...
                                    <div style={{ display: 'flex', gap: 6 }}>
                                        ...
                                        <button onClick={() => duzenleKesim(k)} style={{ background: '#eff6ff', border: 'none', color: '#2563eb', padding: '6px 8px', borderRadius: 8, cursor: 'pointer', fontWeight: 700, fontSize: '0.72rem' }}>✏️</button>
