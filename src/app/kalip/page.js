...
export default function KalipSayfasi() {
    ...
    const sil = async (tablo, id) => {
        if (!kullanici || kullanici.grup !== 'tam') {
            ...
            if (adminPin !== kilitPin) return goster('Yetkisiz İşlem!', 'error');
        }
    ...
    const DURUM_RENK = { taslak: '#94a3b8', kumas_secildi: '#3b82f6', kalip_hazir: '#f59e0b', numune_onay_bekliyor: '#8b5cf6', uretim_hazir: '#10b981', iptal: '#ef4444' };
                        ...
                        {modeller.map(m => (
                            <div key={m.id} style={{ background: 'white', border: '2px solid #f1f5f9', borderRadius: 14, padding: '1.25rem', boxShadow: '0 2px 8px rgba(0,0,0,0.04)', transition: 'all 0.2s' }}>
                                ...
                                <div style={{ display: 'flex', gap: '0.375rem', marginTop: '0.5rem', flexWrap: 'wrap' }}>
                                    ...
                                    <span style={{ fontSize: '0.7rem', background: '#eff6ff', color: '#2563eb', padding: '2px 8px', borderRadius: 4, fontWeight: 600 }}>{m.hedef_kitle}</span>
                                ...
                                <div key={k.id} style={{ background: 'white', border: '2px solid #f1f5f9', borderRadius: 14, padding: '1.25rem', display: 'flex', gap: '1.5rem', alignItems: 'flex-start', boxShadow: '0 2px 8px rgba(0,0,0,0.04)' }}>
                                    <div style={{ flex: 1 }}>
                                        ...
                                        <div style={{ display: 'flex', gap: '1rem', fontSize: '0.78rem', color: '#64748b', fontWeight: 600, alignItems: 'center' }}>
                                            ...
                                            {k.kalip_dosya_url && <a href={k.kalip_dosya_url} target="_blank" rel="noreferrer" style={{ textDecoration: 'none', background: '#dbeafe', color: '#1d4ed8', padding: '2px 8px', borderRadius: 4, fontWeight: 700 }}>⬇️ Kalıbı İndir</a>}
                                        </div>
                                    </div>
