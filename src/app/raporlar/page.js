...
const formatTarih = (iso) => { if (!iso) return 'ÔÇö'; const d = new Date(iso); return d.toLocaleDateString('tr-TR'); };

// CSV Export yard─▒mc─▒ fonksiyonu
...
export default function RaporlarSayfasi() {
    ...
    const yukle = async () => {
        ...
        try {
            ...
            const pRapor = (pList || []).map(p => {
                ...
                const devamlil─▒k = toplamKayit > 0 ? Math.round((devam.calisti / toplamKayit) * 100) : 100;
                return { ...p, gunluk, aylik: gunluk * 22, devam, devamlil─▒k };
            });
    ...
    const DURUM_LABEL = { beklemede: 'Beklemede', onaylandi: 'Onayland─▒', hazirlaniyor: 'Haz─▒rlan─▒yor', kargoda: 'Kargoda', teslim: 'Teslim', iptal: '─░ptal' };
    const MAL_LABEL = { personel_iscilik: 'Personel ─░┼ş├ğilik', isletme_gideri: '─░┼şletme Gideri', sarf_malzeme: 'Sarf Malzeme', fire_kaybi: 'Fire Kayb─▒' };
    const MAL_RENK = { personel_iscilik: '#3b82f6', isletme_gideri: '#f59e0b', sarf_malzeme: '#10b981', fire_kaybi: '#ef4444' };

    const SEKMELER = [
        { id: 'genel', label: '­şôè Genel ├ûzet' },
        { id: 'birim_maliyet', label: '­şÆ░ Birim Maliyet' },
        { id: 'pl', label: '­şôê Kar & Zarar' },
        { id: 'siparisler', label: '­şøı´©Å Sipari┼şler' },
        { id: 'personel', label: '­şæÀ Personel' },
    ];
    ...
    if (!yetkiliMi) {
        return (
            <div dir={isAR ? 'rtl' : 'ltr'} style={{ padding: '3rem', textAlign: 'center', background: '#fef2f2', border: '2px solid #fecaca', borderRadius: '16px', margin: '2rem' }}>
                ...
                <p style={{ color: '#7f1d1d', fontWeight: 600, marginTop: 8 }}>T├╝m ┼şirket raporlar─▒ ve bilan├ğolar─▒ Kararg├óh taraf─▒ndan gizlenmi┼ştir. G├Âr├╝nt├╝lemek i├ğin ├£retim P─░N giri┼şi zorunludur.</p>
            </div>
        );
    }
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1.25rem', flexWrap: 'wrap', gap: 10 }}>
                ...
                <div style={{ display: 'flex', gap: 8, alignItems: 'center', flexWrap: 'wrap' }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 6, background: 'white', border: '2px solid #e5e7eb', borderRadius: 10, padding: '6px 12px' }}>
                        ...
                        <span style={{ color: '#94a3b8', fontSize: '0.75rem' }}>ÔÇö</span>
                    ...
                    <button onClick={() => { setBaslangic(''); setBitis(''); }}
                        ...
                        <RefreshCw size={13} /> S─▒f─▒rla
                    </button>
                    <button
                        onClick={async () => {
                            if (indiriyor) return goster('ÔÅ│ Excel dosyas─▒ haz─▒rlan─▒yor, l├╝tfen bekleyin...', 'error');
                            setIndiriyor(true);
                            ...
                            try {
                                if (aktifSekme === 'siparisler') {
                                    ...
                                    csvIndir(['Ad Soyad', 'Rol', 'G├╝nl├╝k (Ôé║)', 'Ayl─▒k (Ôé║)', 'Devam %', 'Gelmedi'], personelRapor.map(p => [p.ad_soyad, p.rol, p.gunluk.toFixed(0), p.aylik.toFixed(0), p.devamlil─▒k, p.devam.gelmedi || 0]), 'personel');
                                    basari = true;
                                } else if (aktifSekme === 'pl') {
                                    csvIndir(['Kalem', 'Tutar (Ôé║)'], [['Toplam Gelir', plRaporu.gelir.toFixed(2)], ['Toplam Gider', plRaporu.gider.toFixed(2)], ['Net Kar/Zarar', (plRaporu.kar || 0).toFixed(2)], ['Kar Marj─▒ %', plRaporu.marj]], 'kar_zarar');
                                    basari = true;
                                } else {
                                    alert('L├╝tfen daha ├Ânce bir sekme se├ğin (Siparisler, Birim Maliyet, Personel veya Kar&Zarar)');
                                }
                                if (basari) telegramBildirim(`­şÜ¿ D─░KKAT!\nKararg├óh Raporlar─▒ Excel formatiyla indirildi.\nSekme: ${aktifSekme}\nSisteme s─▒zma (Veri ├ç─▒karma) varsa denetleyin.`);
                            } finally {
                                setTimeout(() => setIndiriyor(false), 3000); // 3 saniye anti-spam (U Kriteri) m├╝himmat kalkan─▒
                            }
                        }}
                        disabled={indiriyor}
                    ...
                    {/* CC Kriteri Otomatik Rota (Karargaha D├Ân├╝┼ş - Audit Zincirinin Sonu) */}
                ...
                <div>
                    <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill,minmax(160px,1fr))', gap: '0.75rem', marginBottom: '1.5rem' }}>
                        {[
                            ...
                            { label: 'Kuma┼ş', val: loading ? '...' : kumaslar, icon: Package, color: '#0891b2', bg: '#ecfeff' },
                            { label: 'Sipari┼ş', val: loading ? '...' : siparis, icon: ShoppingCart, color: '#f97316', bg: '#fff7ed' },
                            { label: 'Personel', val: loading ? '...' : personel, icon: Users, color: '#059669', bg: '#ecfdf5' },
                            { label: '­şÅ¡ Aktif ├£retim', val: loading ? '...' : (aktifUretim ?? 0), icon: BarChart2, color: '#8b5cf6', bg: '#f5f3ff' },
                            { label: 'Teslim Ciro', val: loading ? '...' : `Ôé║${(toplamCiro || 0).toFixed(0)}`, icon: DollarSign, color: '#10b981', bg: '#ecfdf5' },
                        ].map((s, i) => (
                    ...
                    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1rem' }}>
                        {durumSay && Object.keys(durumSay).length > 0 && (
                            <div style={{ background: 'white', border: '1px solid #e5e7eb', borderRadius: 16, padding: '1.25rem' }}>
                                <div style={{ fontWeight: 800, color: '#0f172a', fontSize: '0.9rem', marginBottom: '1rem', display: 'flex', alignItems: 'center', gap: 8 }}><BarChart2 size={16} color="#f97316" /> Sipari┼ş Da─ş─▒l─▒m─▒</div>
                        ...
                        {malGrup && Object.keys(malGrup).length > 0 && (
                            <div style={{ background: 'white', border: '1px solid #e5e7eb', borderRadius: 16, padding: '1.25rem' }}>
                                <div style={{ fontWeight: 800, color: '#0f172a', fontSize: '0.9rem', marginBottom: '1rem', display: 'flex', alignItems: 'center', gap: 8 }}><DollarSign size={16} color="#06b6d4" /> Maliyet Da─ş─▒l─▒m─▒</div>
                                {(() => {
                                    ...
                                    return (
                                        <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem' }}>
                                                ...
                                                return (
                                                    <div key={tip}>
                                                        <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 3 }}>
                                                            ...
                                                            <span style={{ fontSize: '0.72rem', fontWeight: 900, color: MAL_RENK[tip] || '#64748b' }}>Ôé║{tutar.toFixed(0)}</span>
                                                        </div>
                                                        <div style={{ height: 6, background: '#f1f5f9', borderRadius: 3, overflow: 'hidden' }}>
                                            ...
                                            <div style={{ marginTop: 8, padding: '8px 12px', background: '#0f172a', borderRadius: 8, display: 'flex', justifyContent: 'space-between' }}>
                                                ...
                                                <span style={{ fontSize: '0.82rem', fontWeight: 900, color: '#34d399' }}>Ôé║{topMal.toFixed(2)}</span>
                                            </div>
                                        </div>
                                    );
                                })()}
            ...
            {aktifSekme === 'birim_maliyet' && (
                <div>
                    <div style={{ background: 'linear-gradient(135deg,#0f172a,#1e293b)', borderRadius: 14, padding: '1rem 1.25rem', marginBottom: '1.25rem', display: 'flex', alignItems: 'center', gap: 12 }}>
                        <div style={{ fontSize: '1.8rem' }}>­şÆ░</div>
                        <div>
                            <div style={{ fontWeight: 900, color: 'white', fontSize: '1rem' }}>├£RET─░LEN ├£R├£N B─░R─░M MAL─░YETLER─░</div>
                            <div style={{ fontSize: '0.75rem', color: '#94a3b8', fontWeight: 600 }}>Muhasebe raporlar─▒ndan otomatik hesapland─▒ ÔÇö Toplam Maliyet ├À Net ├£retilen Adet</div>
                        </div>
                    </div>
                    {birimMaliyetler.length === 0 && (
                        <div style={{ textAlign: 'center', padding: '4rem', background: '#f8fafc', borderRadius: 16, border: '2px dashed #e5e7eb' }}>
                            ...
                            <p style={{ color: '#94a3b8', fontWeight: 700 }}>Hen├╝z kilitlenmi┼ş muhasebe raporu yok.<br />Muhasebe ÔåÆ Rapor ÔåÆ Kilitle i┼şlemi yap─▒ld─▒ktan sonra burada g├Âr├╝n├╝r.</p>
                        </div>
                    )}
                        ...
                        {birimMaliyetler.map(bm => (
                            <div key={bm.id} style={{ background: 'white', border: '2px solid #f1f5f9', borderRadius: 14, padding: '1.125rem 1.25rem', display: 'grid', gridTemplateColumns: '1fr auto auto auto', gap: '1rem', alignItems: 'center' }}>
                                ...
                                <div style={{ textAlign: 'center', background: '#fef9c3', borderRadius: 10, padding: '8px 14px' }}>
                                    ...
                                    <div style={{ fontWeight: 900, color: '#d97706', fontSize: '1.1rem' }}>Ôé║{bm.toplam_maliyet.toFixed(2)}</div>
                                </div>
                                <div style={{ textAlign: 'center', background: 'linear-gradient(135deg,#059669,#047857)', borderRadius: 10, padding: '8px 18px' }}>
                                    <div style={{ fontSize: '0.6rem', color: '#a7f3d0', fontWeight: 700, textTransform: 'uppercase' }}>B─░R─░M MAL─░YET</div>
                                    <div style={{ fontWeight: 900, color: 'white', fontSize: '1.3rem' }}>Ôé║{bm.birim_maliyet.toFixed(2)}</div>
            ...
            {aktifSekme === 'pl' && (
                <div>
                    <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill,minmax(200px,1fr))', gap: '1rem', marginBottom: '1.5rem' }}>
                        {[
                            { label: '­şôÑ Toplam Gelir (Teslim)', val: `Ôé║${plRaporu.gelir.toFixed(2)}`, color: '#10b981', bg: '#ecfdf5', desc: 'Teslim edilen sipari┼şlerin toplam─▒' },
                            { label: '­şôñ Toplam Gider (├£retim)', val: `Ôé║${plRaporu.gider.toFixed(2)}`, color: '#ef4444', bg: '#fef2f2', desc: 'Kay─▒tl─▒ maliyet kalemlerinin toplam─▒' },
                            { label: '­şÆ╣ Net Kar / Zarar', val: `Ôé║${(plRaporu.kar || 0).toFixed(2)}`, color: (plRaporu.kar || 0) >= 0 ? '#10b981' : '#ef4444', bg: (plRaporu.kar || 0) >= 0 ? '#ecfdf5' : '#fef2f2', desc: 'Gelir - Gider' },
                            { label: '­şôè Kar Marj─▒', val: `%${plRaporu.marj}`, color: '#6366f1', bg: '#f5f3ff', desc: 'Net Kar / Gelir ├ù 100' },
                        ].map((k, i) => (
                            ...
                            <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem' }}>
                                {Object.entries(malGrup || {}).map(([tip, tutar]) => (
                                    <div key={tip} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '8px 12px', background: '#f8fafc', borderRadius: 8 }}>
                                        ...
                                        <span style={{ fontWeight: 900, color: MAL_RENK[tip], fontSize: '0.9rem' }}>Ôé║{tutar.toFixed(2)}</span>
                                    </div>
                                ))}
                            </div>
                        </div>
                ...
                <div>
                    {siparislerListesi.length === 0 && (
                        <div style={{ textAlign: 'center', padding: '4rem', background: '#f8fafc', borderRadius: 16, border: '2px dashed #e5e7eb' }}>
                            ...
                            <p style={{ color: '#94a3b8', fontWeight: 700 }}>Se├ğilen tarih aral─▒─ş─▒nda sipari┼ş yok.</p>
                        </div>
                    )}
                    <div style={{ display: 'flex', flexDirection: 'column', gap: '0.375rem' }}>
                        {siparislerListesi.map(s => (
                            <div key={s.id} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '10px 14px', background: 'white', border: '1px solid #f1f5f9', borderRadius: 10 }}>
                                ...
                                <div style={{ fontWeight: 900, color: '#0f172a', fontSize: '0.9rem' }}>Ôé║{parseFloat(s.toplam_tutar_tl || 0).toFixed(2)}</div>
                            </div>
                        ))}
                    </div>
                </div>
            ...
            {aktifSekme === 'personel' && (
                <div>
                    <div style={{ background: 'linear-gradient(135deg,#047857,#065f46)', borderRadius: 14, padding: '1rem 1.25rem', marginBottom: '1.25rem', display: 'flex', alignItems: 'center', gap: 12 }}>
                        <div style={{ fontSize: '1.8rem' }}>­şæÀ</div>
                        <div>
                            <div style={{ fontWeight: 900, color: 'white', fontSize: '1rem' }}>PERSONEL PERFORMANS RAPORU</div>
                            <div style={{ fontSize: '0.75rem', color: '#bae6fd', fontWeight: 600 }}>Aktif personel ÔÇö G├╝nl├╝k ├╝cret, ayl─▒k maliyet ve devam analizi</div>
                        </div>
                    </div>
                        ...
                        {personelRapor.map(p => (
                            <div key={p.id} style={{ background: 'white', border: '2px solid #f1f5f9', borderRadius: 14, padding: '1rem 1.25rem', display: 'grid', gridTemplateColumns: '2fr 1fr 1fr 1fr 1fr', gap: '0.75rem', alignItems: 'center' }}>
                                <div>
                                    ...
                                    <div style={{ fontSize: '0.68rem', color: '#64748b', fontWeight: 600, marginTop: 2 }}>{p.personel_kodu} ÔÇö {(p.rol || '').replace(/_/g, ' ')}</div>
                                </div>
                                <div style={{ textAlign: 'center', background: '#f0fdf4', borderRadius: 10, padding: '8px' }}>
                                    <div style={{ fontSize: '0.58rem', color: '#059669', fontWeight: 700 }}>G├£NL├£K</div>
                                    <div style={{ fontWeight: 900, color: '#059669' }}>Ôé║{p.gunluk.toFixed(0)}</div>
                                </div>
                                <div style={{ textAlign: 'center', background: '#eff6ff', borderRadius: 10, padding: '8px' }}>
                                    ...
                                    <div style={{ fontWeight: 900, color: '#2563eb' }}>Ôé║{p.aylik.toFixed(0)}</div>
                                </div>
                                <div style={{ textAlign: 'center', background: '#f8fafc', borderRadius: 10, padding: '8px' }}>
                                    ...
                                    <div style={{ fontWeight: 900, color: p.devamlil─▒k >= 90 ? '#059669' : p.devamlil─▒k >= 75 ? '#f59e0b' : '#ef4444' }}>%{p.devamlil─▒k}</div>
                                </div>
                                <div style={{ textAlign: 'center', background: p.devam.gelmedi > 2 ? '#fef2f2' : '#f8fafc', borderRadius: 10, padding: '8px' }}>
                                    <div style={{ fontSize: '0.58rem', color: '#64748b', fontWeight: 700 }}>GELMED─░</div>
                                    <div style={{ fontWeight: 900, color: p.devam.gelmedi > 0 ? '#ef4444' : '#059669' }}>{p.devam.gelmedi || 0} g├╝n</div>
                                </div>
                            </div>
                        ))}
                    </div>
                    {personelRapor.length > 0 && (
                        <div style={{ marginTop: '1rem', background: '#0f172a', borderRadius: 12, padding: '1rem 1.25rem', display: 'flex', justifyContent: 'space-between' }}>
                            <span style={{ fontWeight: 800, color: '#94a3b8', fontSize: '0.82rem' }}>TOPLAM AYLIK PERSONEL MAL─░YET─░</span>
                            <span style={{ fontWeight: 900, color: '#34d399', fontSize: '1.1rem' }}>Ôé║{personelRapor.reduce((s, p) => s + p.aylik, 0).toFixed(2)}</span>
                        </div>
                    )}
                </div>
            )}
