...
//
// GET  → 180+ gn hareketsiz kumaş ve stok tespiti
// POST → Aksiyona bağla (fiyat indir, sat ner, iade et)
// ============================================================
...
export async function GET(req) {
    try {
        ...
        const { data: atilKumaslar } = await sb
            .from('b1_kumas_arsivi') // [K2 FIX] Hayalet tablo adı dzeltildi (sonda 'i' eksikti)
        ...
        if (kritikler.length > 0) {
            await sb.from('b1_agent_loglari').insert([{
                ...
                sonuc: 'uyari',
                mesaj: `${tumAtillar.length} hareketsiz kayıt tespit edildi. Bağlı sermaye: ${yapiliBagliSermaye.toLocaleString('tr-TR')} TL. ${kritikler.length} kritik (365+ gn).`,
            }]);
